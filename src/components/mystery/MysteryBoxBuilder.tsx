...
import { Sparkles, Plus, Minus, Check, Gift, Trash2 } from "lucide-react";
 ...
 *  (image, name, ids); per-jersey type+size lives in customerNotes. */
...
export default function MysteryBoxBuilder(props: MysteryBoxBuilderProps) {
  ...
  const addToCart = (buyNow = false) => {
    if (!allConfigured) {
      // Scroll to first unconfigured card
    ...
    setAdding(true);
    // Group identical (type+size) slots so the cart shows e.g. "x3" not 3 lines.
    const groups = new Map<string, { type: MysteryType; size: AdultSize; count: number }>();
    for (const s of slots) {
      if (!s.type || !s.size) continue;
      const key = `${s.type}|${s.size}`;
      const existing = groups.get(key);
      if (existing) existing.count += 1;
      else groups.set(key, { type: s.type, size: s.size, count: 1 });
    }
    for (const g of Array.from(groups.values())) {
      const label =
        MYSTERY_TYPES.find((t) => t.id === g.type)?.labelHe || g.type;
      addItem({
        ...
        image: props.image,
        version: "fan",
        size: g.size as never,
        unitPrice: props.unitPrice,
        customization: null,
        selectedPatchId: null,
        customerNotes: `🎲 Mystery Box · סוג: ${label} · מידה: ${g.size}`,
        quantity: g.count,
      });
    }
    setTimeout(() => {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-body-sm text-muted">
              {formatILS(props.unitPrice)} לחולצה · עד {MAX_QTY} בהזמנה אחת
            </p>
          </div>
          <div className="inline-flex items-center gap-1 rounded-full border border-border bg-surface p-1">
            ...
            <button
              ...
              className="inline-flex h-11 w-11 items-center justify-center rounded-full bg-amber text-background shadow-gold transition-transform hover:-translate-y-0.5 disabled:opacity-30"
            >
      ...
      <div className="space-y-4">
        ...
        {/* Quick "add another" button below the cards too */}
        {quantity < MAX_QTY && (
          <button
            type="button"
            onClick={inc}
      ...
      <div className="sticky bottom-3 z-10 space-y-3 rounded-2xl border border-border bg-surface/95 p-4 shadow-2xl backdrop-blur edge-light">
        ...
        {!allConfigured && (
          ...
          >
            השלימו סוג + מידה לכל החולצות לפני הוספה לסל.
          </p>
        )}
      </div>
    </div>
